...
import { generateStampWithAI, generateCharacterWithAI } from '../utils/aiEngine';
...
export function AIStudio({ onStampCreated, onCharacterCreated }: AIStudioProps) {
  const [activeTab, setActiveTab] = useState<'stamp' | 'character'>('stamp');
  const [isGenerating, setIsGenerating] = useState(false);
  const [generatedItem, setGeneratedItem] = useState<Stamp | AICharacter | null>(null);
  
  // Stamp fields
  const [stampPrompt, setStampPrompt] = useState('');
  
  // Character fields
  const [characterStyle, setCharacterStyle] = useState<AICharacter['style']>('urban');
  ...
  const handleGenerateCharacter = async () => {
    ...
    setGeneratedItem(null);
    try {
      const newCharacter = await generateCharacterWithAI(characterStyle, characterDescription);
      ...
      <div className="container-fd">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-fd-gold/10 border border-fd-gold/30 rounded-full mb-6">
            ...
            <span className="text-fd-gold text-sm tracking-widest uppercase">IA Studio</span>
          </div>
          <h2 className="text-fd-white mb-4">Criação com Inteligência Artificial</h2>
          <p className="text-fd-white/60 max-w-2xl mx-auto">
            Use o poder da IA para criar estampas e personagens exclusivos para suas peças.
          </p>
        </div>
                ...
                <>
                  ...
                  <div className="bg-fd-gray/50 border border-fd-gray-lighter rounded-lg p-4">
                    ...
                    <ul className="text-sm text-fd-white/60 space-y-2">
                      <li>• Seja específico sobre o estilo (minimalista, street, futurista)</li>
                      <li>• Inclua cores principais (dourado, preto, branco)</li>
                      <li>• Mencione elementos visuais (geométrico, orgânico, abstrato)</li>
                      <li>• Defina o mood (autoritário, elegante, agressivo)</li>
                    </ul>
                  </div>
                ...
                <>
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {(['urban', 'futuristic', 'classic', 'dynamic', 'premium'] as const).map((style) => (
                        <button
                          key={style}
